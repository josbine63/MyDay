...
struct ContentView: View {
    ...
    @Binding var quoteOfTheDay: String
...
//    @State private var currentDateKey: String = ""
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            if !hasInitialized {
                Task {
                    try? await photoManager.requestPhotoAccess
                    try? await photoManager.loadAvailableAlbums()
...
//                    try? await Task.sleep(nanoseconds: 500_000_000)
                    ...
                    if !defaults.bool(forKey: "hasLaunchedBefore") || storedAlbumName.isEmpty {
                        if let first = photoManager.albumNames.first {
                            ...
                            try? await photoManager.fetchRandomPhoto(fromAlbum: first)
                            print("📸 Premier lancement — album par défaut: \(first)")
                        }
                    } else {
                        albumName = storedAlbumName
                        try? await photoManager.fetchRandomPhoto(fromAlbum: albumName)
                    }
        ...
        .onChange(of: photoManager.albumNames) { _, newAlbums in
            ...
            if !defaults.bool(forKey: "hasLaunchedBefore") || storedAlbumName.isEmpty {
                ...
                defaults.set(true, forKey: "hasLaunchedBefore")
                Task {
                    try? await photoManager.fetchRandomPhoto(fromAlbum: firstAlbum)
            ...
            HStack(spacing: 8) {
                ...
                Button {
                    if photoManager.albumNames.contains(albumName) {
                        ...
                        task {
                            _ = try? await photoManager.fetchRandomPhoto(fromAlbum: albumName)
                        }
                    } else {
        ...
        VStack(alignment: .leading, spacing: 20) {
            if photoManager.albumNames.isEmpty {
                ...
                .onChange(of: albumName) { _, newAlbum in
                    ...
                    Task {
                          try? await photoManager.fetchRandomPhoto(fromAlbum: newAlbum)
                      }
                }
                .padding()
            }
...
//        try? await Task.sleep(nanoseconds: 500_000_000)
    ...
    func ensureDefaultAlbumSelected() async {
